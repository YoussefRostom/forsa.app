...
const LOCATION_PICKER_KEY = 'clinicEditBranchLocationPickerResult';
const DRAFT_KEY = 'clinicEditBranchDraft';
...
export default function ClinicEditBranchScreen() {
  ...
  useEffect(() => {
    const fetchBranch = async () => {
      ...
      try {
        ...
        const targetIndex = currentLocations.findIndex((location: any, idx: number) => String(location.id || `clinic-branch-${idx}`) === branchId);
        ...
        const [pendingDraftRaw, pickedRaw] = await Promise.all([
          ...
          AsyncStorage.getItem(LOCATION_PICKER_KEY),
        ]);

        if (!isEditMode && !pendingDraftRaw && !pickedRaw) {
          setDraft((prev) => ({ ...prev, phone: prev.phone || String(data.phone || '') }));
          return;
        }
        ...
        if (pickedRaw) {
          ...
          await AsyncStorage.removeItem(LOCATION_PICKER_KEY);
          await AsyncStorage.removeItem(DRAFT_KEY);
        }
    ...
    React.useCallback(() => {
      ...
      const consumePickedLocation = async () => {
        try {
          ...
          if (storedPicked) {
            const picked = normalizePickedLocation(JSON.parse(storedPicked));
            setDraft((prev) => ({
              ...prev,
  ...
  const setField = (key: keyof BranchDraft, value: string) => setDraft((prev) => ({ ...prev, [key]: value }));
  ...
  const openMapPicker = async () => {
    ...
    router.push({
      ...
      params: {
        storageKey: LOCATION_PICKER_KEY,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        id: editingIndex >= 0 ? (nextLocations[editingIndex]?.id || branchId) : `clinic-branch-${Date.now()}`,
      ...
      if (editingIndex >= 0) nextLocations[editingIndex] = payload;
      else nextLocations.push(payload);
      await safeUpdate('clinics', uid, { locations: nextLocations, updatedAt: new Date().toISOString() });
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <TouchableOpacity style={styles.backButton} onPress={() => (router.canGoBack() ? router.back() : router.replace('/clinic-branches' as any))}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.fieldGroup}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (!draft.city) return Alert.alert(i18n.t('selectCityFirst') || 'Select city first');
                if (!isDistrictEnabled) return Alert.alert(i18n.t('noDistrictsAvailable') || 'No districts available');
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.mapPickerButton} onPress={openMapPicker}>
          ...
          <View style={styles.mapStateRow}>
            ...
            <Text style={styles.mapStateText}>{hasMapSelection ? (i18n.t('mapPinSelected') || 'Location pinned successfully') : (i18n.t('mapPinNotSelected') || 'No map pin selected yet')}</Text>
          </View>
        </View>
            ...
            <View style={styles.modalContent}>
              ...
              <ScrollView style={styles.modalScroll}>
                {cityOptions.map(([key, label]) => (
                  <TouchableOpacity key={key} style={styles.modalItem} onPress={() => { setField('city', key); if (!((districtsByCity[key] || []).some((d) => d.key === draft.district))) setField('district', ''); setShowCityModal(false); }}>
...
function Field({
  ...
  return (
    <View style={styles.fieldGroup}>
      ...
      <TextInput style={[styles.input, multiline && styles.inputMultiline]} value={value} onChangeText={onChangeText} multiline={multiline} keyboardType={keyboardType || 'default'} placeholder={label} placeholderTextColor="#9ca3af" />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButton: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 8, borderRadius: 10, backgroundColor: '#fff' },
  backButtonText: { color: '#111827', fontSize: 14, fontWeight: '700' },
  title: { fontSize: 26, fontWeight: '800', color: '#111827' },
  card: { backgroundColor: '#fff', borderRadius: 16, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: '#111827', marginBottom: 6 },
  fieldGroup: { marginTop: 10 },
  label: { fontSize: 13, fontWeight: '600', color: '#374151', marginBottom: 6 },
  input: { borderWidth: 1, borderColor: '#e5e7eb', backgroundColor: '#f9fafb', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 12, color: '#111827', fontSize: 15 },
  inputMultiline: { minHeight: 84, textAlignVertical: 'top' },
  placeholder: { color: '#9ca3af' },
  disabledInput: { opacity: 0.65 },
  mapPickerButton: { marginTop: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderWidth: 1, borderColor: '#d1d5db', backgroundColor: '#f9fafb', paddingVertical: 11, gap: 8 },
  ...
  saveButton: { backgroundColor: '#111827', borderRadius: 14, height: 54, alignItems: 'center', justifyContent: 'center', marginTop: 4 },
  saveButtonDisabled: { opacity: 0.65 },
  saveButtonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'center', alignItems: 'center' },
  modalContent: { width: '90%', maxHeight: '70%', backgroundColor: '#fff', borderRadius: 16, paddingVertical: 10 },
  modalTitle: { fontSize: 16, fontWeight: '800', color: '#111827', paddingHorizontal: 16, paddingBottom: 10 },
  modalScroll: { paddingHorizontal: 8 },
  modalItem: { paddingVertical: 12, paddingHorizontal: 10, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
